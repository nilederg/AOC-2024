...
public class ArrayPoint extends Point2 {
    ...
    public ArrayPoint(long i, long j, int iSize, int jSize) {
        if (i > Integer.MAX_VALUE || j > Integer.MAX_VALUE) {
            throw new ArrayIndexOutOfBoundsException();
        }
        this((int) i, (int) j, iSize, jSize);
    }
    ...
    public static <T> ArrayPoint fromArray(Point2 point, T[][] array) {
        ...
        return new ArrayPoint((int) point.y, (int) point.x, array.length, array[0].length);
    }
